...
class YOLO(mm.MicroMind):
    def __init__(self, m_cfg, hparams, data_cfg_path_val, *args, **kwargs):
...
def modify_yaml(path, key, new_value):
    """Modify .yaml by changing val path and return path to new .yaml"""

    with open(path) as f:
        doc = yaml.load(f, Loader=Loader)

    doc[key] = str(new_value)
...
if __name__ == "__main__":
    ...
    yolo_mind = YOLO(m_cfg, hparams=hparams, data_cfg_path_val = data_cfg_new_path)
